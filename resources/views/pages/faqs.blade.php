...
@push('schemas')
  ...
  @if(!empty($_faqItems))
    <script
      type="application/ld+json">{!! json_encode($_faqSchema, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}</script>
  @endif
@endpush
...
@push('styles')
  <style>
    .faq-hero {
      background:
        radial-gradient(1200px 500px at 10% -20%, rgba(56, 189, 248, 0.35), transparent 60%),
        radial-gradient(900px 500px at 100% 0%, rgba(37, 99, 235, 0.4), transparent 55%),
        linear-gradient(135deg, #1d4ed8 0%, #1e40af 45%, #1e3a8a 100%);
    }
    ...
    .faq-card:hover {
      ...
      box-shadow: 0 18px 40px -24px rgb(124 58 237 / .35);
    }

    .faq-card.is-open {
      border-color: rgb(139 92 246 / .55);
      box-shadow: 0 20px 50px -25px rgb(124 58 237 / .4);
    }
...
@section('content')
  <section class="faq-hero text-white relative overflow-hidden">
    ...
    <div class="relative max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28 text-center">
      <span
        class="inline-flex items-center gap-2 bg-white/10 ring-1 ring-white/20 backdrop-blur px-5 py-2 rounded-full text-sm font-semibold mb-6">
        <i class="fa-solid fa-circle-question text-sky-200"></i>
        {{ __('app.faq_badge') ?: 'Help Center' }}
      </span>
      <h1
        class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight mb-5 bg-gradient-to-br from-white via-white to-sky-200 bg-clip-text text-transparent">
      ...
      <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-6 text-sm">
        <div class="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 ring-1 ring-white/15">
          <i class="fa-solid fa-headset text-sky-200"></i> 24/7 Support
        </div>
        <div class="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 ring-1 ring-white/15">
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 -mt-6 pb-20">
      ...
      <div class="relative mb-8">
        <input type="text" id="faqSearch" placeholder="Search questions, keywords…"
          class="w-full bg-white border border-slate-200 focus:border-sky-500 focus:ring-4 focus:ring-sky-500/15 rounded-2xl py-4 ps-14 pe-6 text-base outline-none shadow-lg shadow-slate-900/5 transition">
        <i
          class="fa-solid fa-magnifying-glass absolute start-6 top-1/2 -translate-y-1/2 text-slate-400 faq-search-icon"></i>
      </div>
      ...
      @if($categories->count() > 1)
        <div class="flex flex-wrap gap-2 mb-8" id="faqChips">
          <button type="button"
            class="chip active px-4 py-2 rounded-full text-sm font-semibold border border-slate-200 bg-white text-slate-700"
            data-cat="all">
            <i class="fa-solid fa-layer-group me-1.5"></i> All
          </button>
          @foreach($categories as $cat)
            <button type="button"
              class="chip px-4 py-2 rounded-full text-sm font-semibold border border-slate-200 bg-white text-slate-700"
              data-cat="{{ $cat }}">
              {{ $cat }}
            </button>
          @endforeach
          ...
          @foreach($_group as $faq)
            ...
            <div class="faq-card faq-item bg-white rounded-2xl border border-slate-200/80 overflow-hidden"
              ...
              <button type="button" class="faq-toggle w-full text-start px-5 sm:px-6 py-5 flex items-center gap-4">
                <span
                  class="faq-icon flex-none w-10 h-10 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center text-sm font-bold transition-colors">
      ...
      {{-- Still need help CTA --}}
      <div
        class="mt-14 rounded-3xl overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-8 sm:p-10 grid sm:grid-cols-[1fr_auto] gap-6 items-center">
        <div>
        ...
        <div class="flex flex-wrap gap-3">
          <a href="{{ route('contact') }}"
            class="inline-flex items-center gap-2 bg-white text-slate-900 hover:bg-sky-50 font-bold px-6 py-3 rounded-xl transition">
