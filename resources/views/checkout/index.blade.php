...
@section('content')
  <main class="bg-slate-50 min-h-screen">
    {{-- Page hero --}}
    <div
      class="relative overflow-hidden bg-gradient-to-b from-blue-700 via-blue-800 to-indigo-950 text-white border-b border-white/10 antialiased">

      <div
        class="absolute top-0 right-1/4 w-[600px] h-[350px] bg-sky-400/20 rounded-full blur-[140px] pointer-events-none">
      </div>
      <div
        class="absolute -bottom-20 left-10 w-[450px] h-[450px] bg-blue-500/15 rounded-full blur-[120px] pointer-events-none">
      </div>

      <div
        class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 flex items-center justify-between gap-6 flex-wrap">

        <div class="flex items-center gap-5 min-w-0">
          <a href="{{ route('products.index') }}"
            class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 hover:border-white/40 hover:bg-white/20 hover:text-sky-300 text-white transition-all duration-200 group shrink-0 shadow-lg shadow-blue-950/20">
            <i class="fa-solid fa-arrow-left transition-transform duration-200 group-hover:-translate-x-0.5"></i>
          </a>

          <div class="min-w-0">
            <nav class="text-[11px] font-bold tracking-wider uppercase text-blue-200/70 mb-1.5 flex items-center gap-2">
              <a href="{{ route('home') }}" class="hover:text-sky-300 transition-colors">Home</a>
              <i class="fa-solid fa-chevron-right text-[8px] text-blue-300/50"></i>
              <span class="text-sky-300 font-black">Checkout</span>
            </nav>
            <h1 class="text-2xl sm:text-4xl font-black tracking-tight text-white flex items-center gap-2 drop-shadow-sm">
              Secure Checkout
            </h1>
          </div>
        </div>

        <div class="flex items-center gap-2.5 text-xs font-bold sm:mt-0 mt-2">
          <span
            class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-2.5 rounded-xl shadow-md shadow-blue-950/10">
            <i class="fa-solid fa-lock text-sky-300 text-sm"></i>
            <span class="tracking-wide">Encrypted</span>
          </span>

          <span
            class="inline-flex items-center gap-2 bg-emerald-400/20 backdrop-blur-md border border-emerald-400/30 text-emerald-300 px-4 py-2.5 rounded-xl shadow-md shadow-blue-950/10">
            <i class="fa-solid fa-shield-halved text-sm animate-pulse"></i>
            <span class="tracking-wide">SSL Secured</span>
          </span>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div id="checkout-empty" class="hidden text-center py-20 bg-white rounded-3xl border border-slate-200">
        ...
        <a href="{{ route('products.index') }}"
          class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl transition-colors">
        ...
        <div class="lg:col-span-3 space-y-6">
          <form id="checkout-form" class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
            <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
              <i class="fa-solid fa-truck text-blue-600"></i> Shipping Information
            </h2>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="space-y-1.5">
                ...
                <input type="text" name="first_name" value="{{ $fn }}" required
              ...
              <div class="space-y-1.5">
                ...
                <input type="text" name="last_name" value="{{ $ln }}" required
                  class="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-300 focus:bg-white transition-colors">
              </div>
              ...
              <div class="space-y-1.5 sm:col-span-2">
                ...
                <input type="text" name="address" value="{{ $profile['shipping_address'] ?? '' }}" required
                  class="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-300 focus:bg-white transition-colors">
              </div>
              <div class="space-y-1.5">
              ...
              <div class="space-y-1.5 sm:col-span-2">
                ...
                <p id="unsupported-country" class="hidden text-xs text-rose-600 font-bold mt-1">
                  <i class="fa-solid fa-circle-info ml-1"></i> We do not support your country yet. It will be available
                  soon.
                </p>
              </div>
            </div>
            ...
            @if($carriers->count())
              <div class="border-t border-slate-100 pt-5">
                ...
                <select id="shipping-carrier" name="shipping_carrier_id"
                  class="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-300 focus:bg-white transition-colors">
                  <option value="">— اختر شركة الشحن —</option>
                  @foreach($carriers as $c)
                    <option value="{{ $c->id }}" data-code="{{ $c->code }}" data-cost="{{ $c->default_cost }}">{{ $c->name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div class="border-t border-slate-100 pt-6">
              <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2 mb-4">
                <i class="fa-solid fa-credit-card text-blue-600"></i> Payment Method
              </h2>
              ...
              @else
                <div class="space-y-3">
                  @foreach($paymentGateways as $g)
                    <label
                      class="flex items-start gap-3 p-4 rounded-2xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/30 cursor-pointer transition-colors has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50/40">
                      <input type="radio" name="payment_gateway" value="{{ $g->code }}"
                        data-fees="{{ (float) $g->extra_fees }}" @checked($loop->first)
                      ...
                      <div class="flex-1">
                        ...
                        @if($g->description)
                        <p class="text-xs text-slate-500">{{ $g->description }}</p>@endif
                        @if($g->code === 'paymob' && $g->configValue('PAYMOB_WALLET_ENABLED') === '1')
                          <div class="mt-3 grid grid-cols-2 gap-2 text-xs font-bold" data-paymob-channels>
                            <label
                              class="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 cursor-pointer has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50 has-[:checked]:text-blue-700">
                              <input type="radio" name="paymob_channel" value="card" checked data-parent-gateway="paymob"
                                class="text-blue-600 focus:ring-blue-500">
                              <span>بطاقة</span>
                            </label>
                            <label
                              class="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 cursor-pointer has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50 has-[:checked]:text-blue-700">
                              <input type="radio" name="paymob_channel" value="wallet" data-parent-gateway="paymob"
                                class="text-blue-600 focus:ring-blue-500">
                              <span>محفظة</span>
                            </label>
                          </div>
        ...
        <div class="lg:col-span-2">
          <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm lg:sticky lg:top-28">
            ...
            <div class="mt-5">
              ...
              <div class="flex gap-2">
                <input type="text" id="coupon-code"
                  class="flex-1 h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm uppercase focus:border-blue-300 focus:bg-white outline-none transition-colors disabled:bg-slate-100 disabled:cursor-not-allowed"
                  placeholder="Enter code">
                <button type="button" id="apply-coupon-btn"
                  class="h-11 px-5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl transition-colors text-sm disabled:bg-slate-300 disabled:cursor-not-allowed">Apply</button>
              </div>
              <p id="coupon-message" class="text-xs mt-2 hidden"></p>
              <div id="coupon-discount-notice"
                class="hidden mt-2 flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <button type="button" id="confirm-btn"
              class="w-full mt-6 h-12 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-blue-500/20">
              Confirm Order
            </button>
          </div>
        </div>
                  ...
                  <div class="flex gap-4" data-cart-id="${safeId}">
                      ...
                      <div class="flex-1 min-w-0">
                          ...
                          <div class="flex items-center justify-between gap-3 mt-2">
                              ...
                              <p class="text-sm font-bold text-blue-600">${lineTotal}</p>
                          </div>
                      </div>
                  </div>
              `;
