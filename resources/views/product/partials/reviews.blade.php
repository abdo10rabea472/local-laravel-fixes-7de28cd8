...
<section id="reviews" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="bg-white rounded-3xl border border-slate-200 p-6 lg:p-8">
        <div class="flex items-center justify-between flex-wrap gap-4 mb-6">
            <div>
                <h2 class="text-2xl font-black text-slate-900">Reviews & Ratings</h2>
                @if($count > 0)
                <div class="flex items-center gap-3 mt-2">
                    ...
                    <span class="text-slate-400 text-sm">({{ $count }} {{ $count === 1 ? 'review' : 'reviews' }})</span>
                </div>
                @else
                <p class="text-sm text-slate-500 mt-1">No reviews yet. Be the first to review!</p>
                @endif
            </div>
        </div>
        ...
        @auth
            <form method="POST" action="{{ route('account.reviews.store') }}" class="bg-slate-50 p-5 rounded-2xl mb-6">
                ...
                <h3 class="font-bold mb-3">{{ $userReview ? 'Edit your review' : 'Add your review' }}</h3>
                <div class="mb-3" x-data="{ rating: {{ $userReview->rating ?? 5 }} }">
                    <label class="text-sm text-slate-600 block mb-1">Rating</label>
        ...
        <div class="space-y-4">
            ...
            <div class="border border-slate-100 rounded-2xl p-4">
                <div class="flex items-start justify-between gap-3 mb-2">
                    <div>
                        <div class="font-bold text-slate-800">{{ $r->user?->name ?? $r->reviewer_name ?? 'Customer' }}</div>
