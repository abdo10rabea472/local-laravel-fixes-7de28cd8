...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    @php
        $cards = [
            ['fa-receipt', __('app.acc_total_orders'), $ordersCount, 'from-blue-500 to-indigo-600'],
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-blue-50/50 to-transparent">
        <h3 class="font-black text-slate-900 flex items-center gap-2">
            <i class="fa-solid fa-clock-rotate-left text-blue-600"></i> {{ __('app.acc_recent_orders') }}
        </h3>
        <a href="{{ route('account.orders') }}" class="text-xs text-blue-600 font-bold hover:underline">{{ __('app.acc_view_all') }}</a>
    </div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody>
            ...
            <tr class="border-t border-slate-100 hover:bg-blue-50/40 transition">
                <td class="p-3 font-mono font-bold text-blue-700">{{ $o->order_number }}</td>
                ...
                <td class="p-3 text-center"><a href="{{ route('account.orders.show', $o) }}" class="text-blue-600 text-xs font-bold hover:underline">{{ __('app.acc_details') }}</a></td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="p-12 text-center">
        <div class="w-16 h-16 rounded-2xl bg-blue-50 text-blue-600 grid place-items-center text-2xl mx-auto mb-3">
            <i class="fa-solid fa-box-open"></i>
        </div>
        <p class="text-slate-500 mb-4">{{ __('app.acc_no_orders_yet') }}</p>
        <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl shadow-md shadow-blue-500/30 hover:opacity-90">
