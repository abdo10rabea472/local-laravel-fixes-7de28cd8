...
<aside class="lg:sticky lg:top-32 lg:w-64 mb-6 lg:mb-0 z-30 self-start">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="bg-gradient-to-br from-sky-600 to-indigo-700 p-5 text-white">
        ...
        <nav class="p-3 space-y-1 text-sm">
            ...
            @foreach($items as [$r,$i,$l])
                <a href="{{ route($r) }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold transition-all
                    {{ request()->routeIs($r) ? 'bg-gradient-to-r from-sky-600 to-indigo-600 text-white shadow-md shadow-sky-500/30' : 'text-slate-600 hover:bg-sky-50 hover:text-sky-700' }}">
                    <i class="fa-solid {{ $i }} w-5 {{ request()->routeIs($r) ? '' : 'text-sky-500' }}"></i> {{ $l }}
                </a>
            @endforeach
