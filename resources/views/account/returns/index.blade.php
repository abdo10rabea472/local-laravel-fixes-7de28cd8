    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-violet-50/50 to-transparent">
            <h1 class="font-black text-slate-900 flex items-center gap-2">
                <i class="fa-solid fa-rotate-left text-violet-600"></i> مرتجعاتي
            </h1>
            <span class="text-xs font-bold text-slate-500">{{ $returns->total() }} طلب إرجاع</span>
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($returns as $r)
                    <tr class="hover:bg-violet-50/40 transition">
                        ...
                        <td class="px-4 py-3 font-bold">{{ number_format($r->refund_amount, 2) }} ج.م</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ route('account.returns.show', $r) }}" class="inline-flex items-center gap-1 text-violet-600 font-bold text-xs hover:underline">عرض <i class="fa-solid fa-arrow-left text-[9px]"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="text-center py-12">
                        ...
                        <p class="text-slate-500">لا توجد مرتجعات.<br><a href="{{ route('account.orders') }}" class="text-violet-600 font-bold hover:underline">اطلب إرجاع من صفحة طلباتك</a></p>
                    </td></tr>
                @endforelse
            </tbody>
        </table>
