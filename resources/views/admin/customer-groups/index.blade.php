...
<div class="p-6 max-w-5xl mx-auto" x-data="{ editing: null, form: { name:'', discount_percent:0, description:'', badge_color:'violet', is_active:true } }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-black text-slate-900">Customer Groups</h1>
            <p class="text-sm text-slate-500 mt-1">VIP, Wholesale, and custom discounts per group.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.customer-groups.store') }}" class="bg-white rounded-2xl border border-slate-200 p-4 mb-6 grid grid-cols-1 md:grid-cols-5 gap-3">
        @csrf
        <input type="text" name="name" placeholder="Group name (e.g. VIP)" required class="h-10 px-3 border border-slate-200 rounded-xl text-sm">
        <input type="number" step="0.01" min="0" max="100" name="discount_percent" placeholder="Discount %" required class="h-10 px-3 border border-slate-200 rounded-xl text-sm">
        ...
        <input type="text" name="description" placeholder="Description (optional)" class="h-10 px-3 border border-slate-200 rounded-xl text-sm">
        <button class="h-10 px-4 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-sm font-bold">Add</button>
    </form>

    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600 text-xs"><tr>
                <th class="p-3 text-left">Group</th><th class="p-3">Discount</th><th class="p-3">Customers</th><th class="p-3">Status</th><th class="p-3">Actions</th>
            </tr></thead>
            <tbody>
                @forelse($groups as $g)
                <tr class="border-t border-slate-100">
                    ...
                    <td class="p-3 text-center">
                        @if($g->is_active)<span class="text-xs text-emerald-700">Active</span>@else<span class="text-xs text-rose-700">Disabled</span>@endif
                    </td>
                    <td class="p-3 text-center">
                        <form method="POST" action="{{ route('admin.customer-groups.update', $g) }}" class="inline" x-data="{ open:false }">
                            @csrf @method('PUT')
                            <button type="button" @click="open=!open" class="text-violet-600 text-xs font-bold">Edit</button>
                            <div x-show="open" x-cloak class="text-left p-3 bg-slate-50 rounded-xl mt-2 space-y-2" @click.stop>
                                ...
                                <label class="text-xs flex gap-1"><input type="checkbox" name="is_active" value="1" @checked($g->is_active)> Active</label>
                                <button class="w-full h-9 bg-violet-600 text-white rounded-lg text-sm font-bold">Save</button>
                            </div>
                        </form>
                        <form method="POST" action="{{ route('admin.customer-groups.destroy', $g) }}" class="inline" onsubmit="return confirm('Confirm delete?')">
                            @csrf @method('DELETE')
                            <button class="text-rose-600 text-xs font-bold ml-2">Delete</button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr><td colspan="5" class="p-8 text-center text-slate-400">No groups yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
