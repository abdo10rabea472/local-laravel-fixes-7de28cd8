    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">
                Order: <a href="{{ route('admin.orders.show', $return->order_id) }}" class="text-violet-600 font-semibold hover:underline">{{ $return->order?->order_number }}</a>
            </p>
        </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border p-6">
                <h2 class="font-bold text-slate-800 mb-4">Return Items</h2>
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        @foreach($return->items as $item)
                            <tr>
                                <td class="py-3 font-medium text-slate-800">{{ $item->product?->name ?? '(deleted)' }}</td>
            ...
            <div class="bg-white rounded-2xl shadow-sm border p-6">
                <h2 class="font-bold text-slate-800 mb-3">Return Reason</h2>
                ...
                @if($return->customer_note)
                    <div class="bg-slate-50 rounded-xl p-4 mt-3">
                        <p class="text-xs text-slate-500 mb-1 font-semibold">Customer note:</p>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border p-6">
                <h2 class="font-bold text-slate-800 mb-4">Current Status</h2>
                ...
                <form method="POST" action="{{ route('admin.returns.status', $return) }}" class="mt-6 space-y-4">
                    @csrf @method('PATCH')
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Change To</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Admin Note</label>
            ...
            <div class="bg-white rounded-2xl shadow-sm border p-6">
                ...
                @if($return->pickup_guid)
                    <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs space-y-1">
                        <p class="font-bold text-emerald-700">✓ Pickup scheduled</p>
                ...
                @else
                    <p class="text-xs text-slate-500 mb-3">Schedule a pickup from the customer via Aramex.</p>
