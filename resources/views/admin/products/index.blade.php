...
@section('content')
<x-admin.page title="Store Products" subtitle="Manage products with categories and multiple images.">
    {{-- ============ MAIN ============ --}}
    <x-admin.card title="All Products" icon="fa-boxes" padding="p-0">
        <form method="POST" action="{{ route('admin.products.bulk-action') }}" id="bulk-products-form">
            ...
            <div class="flex flex-wrap items-center gap-2 p-3 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-dark-800/40">
                ...
                <button type="submit" onclick="return confirm('Apply action to selected products?')"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-dark-800/50">
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        @if($product->featured)<span class="text-[10px] text-amber-600 font-bold">Featured</span>@endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="p-4 font-bold text-gray-900 dark:text-white">{{ number_format($product->sale_price ?? $product->price, 2) }} EGP</td>
                            <td class="p-4">
                            ...
                            <td class="p-4">
                                <div class="flex gap-3">
                                    <a href="{{ route('admin.products.edit', $product) }}" class="text-primary-600 hover:text-primary-700" title="Edit"><i class="fa-solid fa-pen"></i></a>
                                    <a href="{{ route('product.show', $product->slug) }}" target="_blank" class="text-gray-500 hover:text-gray-700" title="View"><i class="fa-solid fa-eye"></i></a>
                                    <button type="submit" form="dup-{{ $product->id }}" class="text-sky-600 hover:text-sky-700" title="Duplicate"><i class="fa-solid fa-clone"></i></button>
                                    <button type="submit" form="del-{{ $product->id }}" onclick="return confirm('Delete product?')" class="text-rose-500 hover:text-rose-600" title="Delete"><i class="fa-solid fa-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                        @empty
        ...
        {{-- Standalone delete/duplicate forms (outside bulk form to avoid nesting) --}}
    ...
    <x-slot:side>
        ...
        <x-admin.card title="Filter & search" icon="fa-filter">
            <form method="GET" class="space-y-3">
                ...
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">Category</label>
