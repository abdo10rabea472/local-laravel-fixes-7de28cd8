    ...
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div class="flex gap-2">
            <button @click="toggleActive()" :disabled="busy"
                ...
                <i class="fa-solid {{ $customer->is_active ? 'fa-ban' : 'fa-check' }} mr-1"></i>
                {{ $customer->is_active ? 'Ban' : 'Activate' }}
            </button>
            <a href="{{ route('admin.customers.index') }}" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold">Back</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div class="p-4 border-b border-slate-100 font-bold">Recent Orders</div>
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 text-xs text-slate-600"><tr>
                        <th class="p-3 text-left">Order #</th><th class="p-3">Items</th><th class="p-3">Total</th><th class="p-3">Status</th><th class="p-3">Date</th><th class="p-3"></th>
                    </tr></thead>
                    <tbody>
                        ...
                        <tr class="border-t border-slate-100">
                            ...
                            <td class="p-3 text-center"><a href="{{ route('admin.orders.show', $o) }}" class="text-violet-600 text-xs font-bold">View</a></td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="p-6 text-center text-slate-400 text-sm">No orders.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <h3 class="font-bold mb-3">Customer Reviews</h3>
        ...
        <div class="space-y-6">
            ...
            <form method="POST" action="{{ route('admin.customers.update', $customer) }}" class="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
                ...
                <button class="w-full h-10 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold text-sm">Save</button>
            </form>

            <!-- Send email -->
            <div class="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
                <h3 class="font-bold">Send Email to Customer</h3>
                <input type="text" x-model="subject" placeholder="Subject" class="w-full h-10 px-3 border border-slate-200 rounded-xl text-sm">
                <textarea x-model="body" rows="5" placeholder="Message body..." class="w-full p-3 border border-slate-200 rounded-xl text-sm"></textarea>
                <button @click="sendEmail()" :disabled="busy" class="w-full h-10 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-xl font-bold text-sm">
                    <span x-show="!busy">Send</span><span x-show="busy">Sending...</span>
                </button>
            </div>
        </div>
    </div>
...
function customerShow(id){return{
    ...
    sendEmail(){
        if(!this.subject || !this.body){ this.toast('Fill subject and body', false); return; }
