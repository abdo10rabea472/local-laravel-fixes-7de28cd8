...
@section('content')
<x-admin.page title="Coupons" subtitle="Manage all discount codes available in the store.">
    <x-admin.card title="All Coupons" icon="fa-ticket" padding="p-0">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-dark-800/50">
                        ...
                        <td class="p-3 text-center text-xs text-gray-600 dark:text-gray-400">{{ ['all'=>'All','products'=>'Products','categories'=>'Categories'][$c->scope] }}</td>
                        ...
                        <td class="p-3 text-center">
                            ...
                            @else
                                <span class="px-2 py-1 text-xs bg-gray-100 dark:bg-dark-800 text-gray-600 dark:text-gray-400 rounded-full font-bold">Inactive</span>
                            @endif
                        </td>
                        <td class="p-3 text-center whitespace-nowrap">
                            <a href="{{ route('admin.coupons.edit', $c) }}" class="text-primary-600 hover:underline text-xs font-bold">Edit</a>
                            <form action="{{ route('admin.coupons.toggle', $c) }}" method="POST" class="inline" data-ajax-toggle>
                                @csrf @method('PATCH')
                                <button data-toggle-state="{{ $c->is_active ? 'on' : 'off' }}" data-toggle-on="Disable" data-toggle-off="Enable" class="text-amber-600 hover:underline text-xs font-bold mx-2">{{ $c->is_active ? 'Disable' : 'Enable' }}</button>
                            </form>
                            <form action="{{ route('admin.coupons.destroy', $c) }}" method="POST" class="inline" data-ajax-confirm="Delete coupon?" data-ajax-remove>
                                @csrf @method('DELETE')
    ...
    <x-slot:side>
        ...
        <x-admin.card title="Filter & Search" icon="fa-filter">
            <form method="GET" class="space-y-3">
                <div>
                    <label class="text-xs font-bold text-gray-500 dark:text-gray-400 block mb-1.5">Search by code</label>
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="e.g. WELCOME10"
