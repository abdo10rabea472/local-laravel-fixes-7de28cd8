...
<article class="group flex flex-col rounded-2xl bg-white border border-slate-200/80 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300" data-id="{{ $product->id }}" data-price="{{ $convertedPrice }}">
    <a href="{{ route('product.show', $product->slug) }}" class="relative aspect-square bg-slate-50 flex items-center justify-center p-4 overflow-hidden">
        @if($hasDiscount && $discountPercent > 0)
            <span class="absolute top-4 left-4 z-10 bg-gradient-to-br from-rose-500 to-red-600 text-white font-black text-[11px] px-3 py-1.5 rounded-xl shadow-[0_8px_16px_rgba(244,63,94,0.3),inset_0_2px_4px_rgba(255,255,255,0.2)] border-t border-white/20 transform rotate-6 group-hover:scale-110 group-hover:rotate-0 transition-all duration-300">SAVE -{{ $discountPercent }}%</span>
        @endif
        @if($product->featured ?? false)
            <span class="absolute top-4 right-4 z-10 bg-gradient-to-br from-amber-500 to-orange-600 text-white font-black text-[10px] px-2.5 py-1.5 rounded-xl shadow-[0_8px_16px_rgba(245,158,11,0.3),inset_0_2px_4px_rgba(255,255,255,0.2)] border-t border-white/20 transform rotate-2 group-hover:scale-110 group-hover:rotate-0 transition-all duration-300">Featured</span>
        @endif
    ...
    <div class="p-4 flex flex-col flex-1">
        @if($product->relationLoaded('category') && $product->category)
            <span class="inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-600 bg-gradient-to-r from-blue-50 to-sky-50 rounded-full border border-blue-100 shadow-sm mb-3">{{ $product->category->name }}</span>
        @endif
        ...
        <div class="mt-auto pt-4 flex items-end justify-between gap-2 border-t border-slate-100 mt-3">
            ...
            @else
                <span class="inline-flex items-center px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-rose-600 bg-rose-50 border border-rose-100 rounded-2xl shadow-sm whitespace-nowrap">
                  {{ __('app.products_card_out_of_stock') }}
              </span>          
            @endif
        </div>
    </div>
